...
export default function MembersPage() {
  ...
  const handleSuspend = (member: MemberDto) => {
    setMemberToManage(member);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm flex flex-col">
        <div className="p-4 border-b border-slate-800 flex flex-col sm:flex-row gap-4 justify-between bg-slate-900/50">
          ...
          <select
            ...
            className="px-4 py-2 bg-slate-950 border border-slate-700 rounded-lg text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm appearance-none min-w-[150px]"
          >
                  ...
                  <tr key={member.id} className="border-b border-slate-800/50 hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full ${
                        member.activeBorrows > 0 ? "bg-indigo-500/20 text-indigo-400 font-bold" : "bg-slate-800 text-slate-500"
                      }`}>
