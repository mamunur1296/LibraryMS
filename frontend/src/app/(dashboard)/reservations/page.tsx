...
export default function ReservationsPage() {
  ...
  const handleViewQueue = (bookId: string, branchId: string) => {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm flex flex-col">
        <div className="p-4 border-b border-slate-800 flex flex-col sm:flex-row justify-between bg-slate-900/50">
          <select
            ...
            className="px-4 py-2 bg-slate-950 border border-slate-700 rounded-lg text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm appearance-none min-w-[200px]"
          >
                    ...
                    <td className="px-6 py-4 text-center">
                      {(reservation.status === "Pending" || reservation.status === "Notified") ? (
                         <span className="inline-flex flex-col items-center justify-center">
                           ...
                           <button 
                             onClick={() => handleViewQueue(reservation.bookId, reservation.branchId)}
                             className="text-[10px] text-slate-400 hover:text-indigo-300 transition-colors mt-1 underline underline-offset-2"
                           >
                             View full queue
                           </button>
                         </span>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col items-start gap-1">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                          ...
                          reservation.status === "Notified" ? "bg-blue-500/10 text-blue-400 border-blue-500/20" :
                          reservation.status === "Fulfilled" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" :
                          "bg-slate-500/10 text-slate-400 border-slate-500/20"
                        }`}>
                          {reservation.status}
                        </span>
                        <span className="text-[10px] text-slate-500 mt-1">
                          Placed: {new Date(reservation.createdAt).toLocaleDateString()}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right whitespace-nowrap">
                      {(reservation.status === "Pending" || reservation.status === "Notified") && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded transition-colors text-xs font-medium"
                        >
                          Cancel
                        </button>
                      )}
