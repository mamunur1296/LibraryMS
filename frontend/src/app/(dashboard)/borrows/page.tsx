...
export default function BorrowsPage() {
  ...
  const handleReturn = (borrow: BorrowDto) => {
    setBorrowToManage(borrow);
  ...
  const handlePayFine = async (id: string) => {
    if (confirm("Confirm fine payment?")) {
      try {
        await borrowService.payFine(id);
        fetchBorrows();
      } catch (error: any) {
                ...
                data.items.map((borrow) => (
                  <tr key={borrow.id} className="border-b border-slate-800/50 hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className={`text-xs mt-1 ${borrow.isOverdue && borrow.status === "Active" ? "text-red-400 font-medium" : "text-slate-400"}`}>
                    ...
                    <td className="px-6 py-4 text-right space-x-3 whitespace-nowrap">
                      {borrow.status === "Active" && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 rounded transition-colors text-xs font-medium"
                        >
                          Process Return
                        </button>
                      )}
                      {borrow.lateFine > 0 && !borrow.isFinePaid && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 rounded transition-colors text-xs font-medium"
                        >
                          Pay Fine
                        </button>
                      )}
