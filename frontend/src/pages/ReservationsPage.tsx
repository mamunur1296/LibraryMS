...
export default function ReservationsPage() {
  ...
  useEffect(() => { fetchReservations(); }, [statusFilter, page]);

  const handleCancel = async (id: string) => {
    if (confirm('Are you sure you want to cancel this reservation?')) {
      try { await reservationService.cancel(id); fetchReservations(); }
      catch (error: any) { alert(error.response?.data?.message || 'Failed to cancel reservation.'); }
    }
  };
  ...
  return (
    <div className="space-y-6">
                ...
                data.items.map((reservation) => (
                  <tr key={reservation.id} className="border-b border-slate-800/50 hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right whitespace-nowrap">
                      {(reservation.status === 'Pending' || reservation.status === 'Notified') && (
                        <button onClick={() => handleCancel(reservation.id)} className="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded text-xs font-medium">Cancel</button>
                      )}
                    </td>
                  </tr>
                ))
      ...
      <ReservationFormModal isOpen={isFormModalOpen} onClose={() => setIsFormModalOpen(false)} onSuccess={fetchReservations} />
