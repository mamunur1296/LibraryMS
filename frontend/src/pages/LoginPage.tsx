...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      // Refresh auth context so Navbar shows user immediately
      await refreshUser();
      navigate('/dashboard');
    } catch (err: unknown) {
      const axiosErr = err as { response?: { data?: { message?: string } }; message?: string };
      setError(
        axiosErr.response?.data?.message || axiosErr.message || 'Invalid username or password'
      );
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900">
      ...
      <div className="relative w-full max-w-md p-8 bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl rounded-3xl z-10">
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-white tracking-tight">Library<span className="text-indigo-400">MS</span></h2>
          <p className="text-slate-300 mt-2 text-sm">Sign in to manage the library</p>
        </div>
